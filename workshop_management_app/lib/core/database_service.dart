...
class DatabaseService {
  ...
  Future<void> _createDB(Database db, int version) async {
    // Employees, Purchases, Inventory, Parts/Assembly tables (as before)
    ...
    await db.execute('''
      CREATE TABLE sales_orders(
        ...
        status TEXT NOT NULL DEFAULT 'Pending', -- e.g., Pending, Confirmed, Awaiting Payment, Shipped, Completed, Cancelled
    ...
    await db.execute('''
      CREATE TABLE order_items(
        ...
        price_at_sale REAL NOT NULL,  -- Price per unit at time of sale
  ...
  // --- SalesOrder and OrderItem CRUD (NEW) ---
  Future<int> insertSalesOrder(SalesOrder order) async {
    final db = await database;
    return await db.transaction((txn) async {
      order.calculateTotalAmount();
      int orderId = await txn.insert('sales_orders', order.toMap());
  ...
  Future<List<SalesOrder>> getSalesOrders({String? status, int? customerId}) async {
    ...
    final List<Map<String, dynamic>> orderMaps = await db.query('sales_orders', where: whereClause, whereArgs: whereArgs.isNotEmpty ? whereArgs : null, orderBy: 'order_date DESC');
  ...
  Future<void> completeSalesOrderAndUpdateInventory(int salesOrderId) async {
    ...
    await db.transaction((txn) async {
      final order = await getSalesOrderById(salesOrderId);
      if (order == null) throw Exception('SalesOrder not found for completion.');
      if (order.status == 'Completed') throw Exception('SalesOrder already completed.');

      for (var item in order.items) {
        final product = await getProductById(item.productId, txn: txn);
        if (product == null) throw Exception('Product with ID ${item.productId} not found.');

        final productParts = await getPartsForProduct(product.id!, txn: txn);
        if (productParts.isEmpty) { // Product itself is a directly inventoried item (not an assembly defined by product_parts)
             await _updateInventoryItemQuantityInternal(product.name, -item.quantity, txn: txn);
        } else { // Product is an assembly defined in product_parts
            for (var productPart in productParts) {
                final partToConsume = await getPartById(productPart.partId, txn: txn);
                if (partToConsume == null) throw Exception('Part with ID ${productPart.partId} for product ${product.name} not found.');

                final totalQuantityOfPartToConsume = productPart.quantity * item.quantity;
                await _updateInventoryItemQuantityInternal(partToConsume.name, -totalQuantityOfPartToConsume, txn: txn);
            }
        }
      }
